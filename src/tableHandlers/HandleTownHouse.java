...
public class HandleTownHouse {
    ...
    public HandleTownHouse(Connection c) {
        this.c = c;
    }

    public void insertTownHouse(String PostalCode, String StreetAddress, String Unit) {
        try {
            String query = "INSERT INTO TownHouse (PostalCode, StreetAddress, Unit) VALUES (?,?,?)";
    ...
    public void deleteTownHouse (String PostalCode, String StreetAddress) {
        try {
            String query = "DELETE FROM TownHouse WHERE Postalcode = ? AND StreetAddress = ?";
            ps = c.prepareStatement(query);
            ...
            if (rowCount == 0) {
                System.out.println("TownHouse " + PostalCode + " " + StreetAddress  + " does not exist!");
            }
            c.commit();
            ps.close();
        } catch (SQLException e) {
    ...
    public void updateTownHouse (ArrayList<UpdateTemplate> updates, String postalCode, String streetAddress) {
        try {
            if (updates.size() == 0) {
                return;
            }
            String searchQuery = "SELECT * FROM TownHouse WHERE PostalCode = ? AND StreetAddress = ?";
            ps = c.prepareStatement(searchQuery);
            ps.setString(1, util.PadSpace(postalCode, PostalCodeLength));
            ps.setString(2, util.PadSpace(streetAddress, StreetAddressLength));
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                String updateQuery = "UPDATE TownHouse Set ";
